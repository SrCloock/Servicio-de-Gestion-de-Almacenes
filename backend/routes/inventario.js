...
module.exports = function createinventarioRouter({ sql, getPool }) {
  ...
  // ============================================================
  // CTEs BASE — incluyen filtro de ubicaciones válidas
  // Un registro de AcumuladoStockUbicacion es válido si su Ubicacion
  // pertenece al mismo CodigoAlmacen en la tabla Ubicaciones.
  // ============================================================
  ...
  // ============================================================
  // LIMPIEZA DE CEROS — borra registros de AcumuladoStockUbicacion
  // con cantidad 0 que NO sean la ubicación principal del artículo.
  // La ubicación principal se lee de AcumuladoStock.Ubicacion (periodo 99).
  // ============================================================
  async function limpiarRegistrosCeroNoPrincipales(codigoEmpresa) {
    const ejercicioActual = new Date().getFullYear();
    console.log(`[LIMPIEZA CEROS] Iniciando para empresa ${codigoEmpresa}, ejercicio ${ejercicioActual}`);
    ...
    const result = await getPool().request()
      ...
      .query(`
        ...
        -- Verificar que la ubicación pertenece al almacén correcto
        INNER JOIN Ubicaciones uv
        ...
        WHERE asu.CodigoEmpresa = @codigoEmpresa
          ...
          -- No es la ubicación principal (la que figura en AcumuladoStock)
          AND NOT EXISTS (
            SELECT 1
  ...
  // ============================================================
  // SINCRONIZACIÓN AUTOMÁTICA — solo limpieza de ceros
  // ============================================================
  async function sincronizacionAutomatica() {
    console.log('[SYNC AUTO] Iniciando limpieza de registros cero no principales...');
    ...
    try {
      ...
      for (const empresa of empresasResult.recordset) {
        try {
          ...
          console.error(`[SYNC AUTO] Error en empresa ${empresa.CodigoEmpresa}:`, error.message);
        }
      }

      console.log(`[SYNC AUTO] Completado: ${totalBorrados} registros cero eliminados en total`);
    } catch (error) {
  ...
  function iniciarSincronizacionAutomatica() {
    console.log('🚀 [SYNC AUTO] Configurando limpieza automática de inventario...');

    setTimeout(() => {
      console.log('⏰ [SYNC AUTO] Ejecutando limpieza inicial...');
      sincronizacionAutomatica();
    }, 5000);

    cron.schedule('0 */3 * * *', () => {
      console.log('⏰ [SYNC AUTO] Ejecutando limpieza programada cada 3 horas...');
      sincronizacionAutomatica();
    });
  ...
  // Esta es la única función que toca AcumuladoStock.
  // Aplica el delta (positivo o negativo) sobre el registro existente,
  ...
  async function aplicarDeltaEnAcumuladoStock(
    ...
    // Si no existía el registro, crearlo con el delta como valor inicial
    if (filasActualizadas === 0) {
      console.log(`[DELTA STOCK] No existe periodo99 en AcumuladoStock para ${articulo}/${codigoAlmacen}, creando...`);
    ...
    console.log(`[DELTA STOCK] ${articulo}/${codigoAlmacen}: delta=${delta}, filasActualizadas=${filasActualizadas}`);
  }

  // ============================================================
  // OBTENER UBICACIÓN PRINCIPAL desde AcumuladoStock
  // ============================================================
    ...
    try {
      ...
      const detalleQuery = `
        WITH StockUbicacionSumado AS (
          ...
          FROM AcumuladoStockUbicacion s
          -- ✅ FILTRO: solo ubicaciones válidas
          INNER JOIN Ubicaciones uv
            ON uv.CodigoEmpresa = s.CodigoEmpresa
            AND uv.CodigoAlmacen = s.CodigoAlmacen
            AND uv.Ubicacion = s.Ubicacion
  ...
  router.get('/inventario/almacenes-ajuste', async (req, res) => {
    ...
    try {
      ...
      console.error('[ERROR ALMACENES AJUSTE]', err);
  ...
  router.get('/stock/por-ubicacion', async (req, res) => {
    ...
    try {
      ...
      // Verificar que la ubicación pertenece al almacén
      ...
      res.json({
        ...
        total,
        debug: { ejercicio: ejercicioActual, periodo: 99, almacen: codigoAlmacen, ubicacion }
      });
    } catch (err) {
    ...
    try {
      ...
      const result = await agregarContextoInventario(
        ...
        StockConUbicacion AS (
          SELECT
            s.CodigoArticulo, s.CodigoAlmacen,
            ISNULL(s.TipoUnidadMedida_, '') AS TipoUnidadMedida_,
            ISNULL(s.Partida, '') AS Partida,
            ISNULL(s.CodigoColor_, '') AS CodigoColor_,
            ISNULL(s.CodigoTalla01_, '') AS CodigoTalla01_,
        ...
        SELECT
          ...
          END AS CantidadBase,
          CONCAT(
            @codigoEmpresa, '_', @ejercicioBase, '_',
            st.CodigoAlmacen, '_', 'SIN_UBICACION', '_',
            st.CodigoArticulo, '_',
      ...
      const dataResult = await getPool().request()
        ...
        .query(`
          WITH Combinados AS (
            ...
            SELECT
              YEAR(i.FechaCreacion) AS Ejercicio, MONTH(i.FechaCreacion) AS Periodo,
              ...
              i.Inventario AS Comentario, i.FechaCreacion AS FechaRegistro,
  ...
  // ============================================================
  // DETALLES POR MOV_POSICION_LINEA
  // ============================================================
    ...
    try {
      const result = await getPool().request()
        ...
        .query(`
          ...
          FROM Articulos
          WHERE CodigoEmpresa = @codigoEmpresa AND CodigoFamilia IS NOT NULL AND CodigoFamilia <> ''
    ...
    try {
      const result = await getPool().request()
        ...
        .query(`
          ...
          FROM Articulos
          WHERE CodigoEmpresa = @codigoEmpresa AND CodigoSubfamilia IS NOT NULL AND CodigoSubfamilia <> ''
    ...
    try {
      ...
      if (incluirSinUbicacion === 'true') {
        query += `
          ...
          FROM Almacenes alm
          WHERE alm.CodigoEmpresa = @codigoEmpresa AND alm.CodigoAlmacen = @codigoAlmacen
        `;
      }
  ...
  router.get('/articulos/:codigoArticulo/variantes-contexto', async (req, res) => {
    ...
    try {
      const articuloResult = await getPool().request()
        ...
        .query(`
          SELECT TOP 1
            CodigoArticulo, DescripcionArticulo, UnidadMedida2_, UnidadMedidaAlternativa_,
      ...
      console.error('[ERROR VARIANTES CONTEXTO]', err);
          ...
          SELECT
            ...
            (SELECT COUNT(*) FROM AcumuladoStockUbicacion s
             WHERE s.CodigoEmpresa = u.CodigoEmpresa
               AND s.CodigoAlmacen = u.CodigoAlmacen
               AND s.Ubicacion = u.Ubicacion
               AND s.Periodo = 99 AND s.UnidadSaldo > 0) AS CantidadArticulos
          FROM Ubicaciones u
          INNER JOIN Almacenes alm ON alm.CodigoEmpresa = u.CodigoEmpresa AND alm.CodigoAlmacen = u.CodigoAlmacen
    ...
    try {
      ...
      let query = `
        ...
        WHERE s.CodigoEmpresa = @codigoEmpresa
          ...
          AND s.Ejercicio IN (@ejercicioBase, @ejercicioActual)
        GROUP BY
          s.CodigoAlmacen, alm.Almacen, s.Ubicacion, u.DescripcionUbicacion,
      ...
      if (incluirSinUbicacion === 'true') {
        query += `
          ...
          SELECT
            ...
            'SIN-UBICACION' AS Ubicacion, 'Stock sin ubicación asignada' AS DescripcionUbicacion,
            (s.StockTotal - ISNULL(u.StockUbicado, 0)) AS Cantidad,
            'unidades' AS UnidadMedida, 'unidades' AS TipoUnidadMedida_,
            '' AS Partida, '' AS CodigoColor_, '' AS Talla, '' AS NombreColor,
  ...
  router.post('/inventario/ajustar-completo', async (req, res) => {
    ...
    try {
      ...
      for (const ajuste of ajustes) {
        ...
        // ✅ Validar que la ubicación destino pertenece al almacén correcto
        ...
        // Obtener ubicación principal para AcumuladoStock
        const ubicacionPrincipal = await obtenerUbicacionPrincipal(
          codigoEmpresa, ejercicio, ajusteDestino.articulo, ajusteDestino.codigoAlmacen, transaction
        );
        ...
        // --- EDICIÓN CON CAMBIO DE COMBINACIÓN ---
        if (esEdicion && !mismaCombinacion) {
          ...
          if (Math.abs(deltaOrigen) > 0.001) {
            await aplicarDeltaEnAcumuladoStock(
              codigoEmpresa, ejercicio, ajusteOrigen.articulo, ajusteOrigen.codigoAlmacen,
              ubicacionPrincipal, ajusteOrigen.unidadStock, ajusteOrigen.partida,
              ajusteOrigen.codigoColor, ajusteOrigen.codigoTalla01, deltaOrigen, transaction
            );
          }
          if (Math.abs(deltaDestino) > 0.001) {
            await aplicarDeltaEnAcumuladoStock(
              codigoEmpresa, ejercicio, ajusteDestino.articulo, ajusteDestino.codigoAlmacen,
              ubicacionPrincipal, ajusteDestino.unidadStock, ajusteDestino.partida,
              ajusteDestino.codigoColor, ajusteDestino.codigoTalla01, deltaDestino, transaction
            );
          }
          ...
          await registrarMovimientoInventario({
            ...
            comentario: registroDestino
              ? `Inventario: fusion de variante`
              : `Inventario: cambio de variante`
          }, transaction);
          continue;
        }

        // --- AJUSTE NORMAL (misma combinación o nueva) ---
        const cantidadAnterior = parseFloat(registroOrigen?.UnidadSaldo ?? registroDestino?.UnidadSaldo ?? 0);
        ...
        // Aplicar delta en AcumuladoStock
        if (Math.abs(delta) > 0.001) {
          await aplicarDeltaEnAcumuladoStock(
            codigoEmpresa, ejercicio, ajusteDestino.articulo, ajusteDestino.codigoAlmacen,
            ubicacionPrincipal, ajusteDestino.unidadStock, ajusteDestino.partida,
            ajusteDestino.codigoColor, ajusteDestino.codigoTalla01, delta, transaction
          );
        }
  ...
  // ============================================================
  // UPSERT AcumuladoStockUbicacion (periodo 99, ejercicio actual)
  // Escribe directamente el valor deseado (no el delta).
  // ============================================================
  async function upsertAcumuladoStockUbicacion(ajuste, nuevaCantidad, codigoEmpresa, ejercicio, transaction) {
    const {
      articulo, codigoAlmacen, ubicacionStr, partida, unidadStock, codigoColor, codigoTalla01
    } = ajuste;

    const ubicacionNorm = ubicacionStr === 'SIN UBICACIÓN' ? 'SIN-UBICACION' : ubicacionStr;
    const unidadNorm = (unidadStock === 'unidades' ? '' : unidadStock || '');

    // Eliminar registro existente periodo 99 (solo ejercicio actual)
    await new sql.Request(transaction)
      .input('codigoEmpresa', sql.SmallInt, codigoEmpresa)
      .input('ejercicio', sql.Int, ejercicio)
      .input('codigoAlmacen', sql.VarChar, codigoAlmacen)
    ...
    // Insertar si la cantidad es distinta de cero
    ...
    console.log(`[UPSERT UBICACIóN] ${articulo}/${codigoAlmacen}/${ubicacionNorm}: ${nuevaCantidad}`);
  }

  // ============================================================
  // UBICACIONES AJUSTE (paginado)
  // ============================================================
  router.get('/inventario/ubicaciones-ajuste', async (req, res) => {
    ...
    try {
      ...
      console.error('[ERROR UBICACIONES AJUSTE]', err);
      res.status(500).json({ success: false, mensaje: 'Error al obtener ubicaciones.', error: err.message });
    }
  });
  ...
  async function obtenerRegistroVigenteAcumuladoStockUbicacionExacto(codigoEmpresa, ajuste, transaction) {
    const result = await new sql.Request(transaction)
      ...
      .query(`
        SELECT TOP 1
          Ejercicio, Periodo, UnidadSaldo, UnidadSaldoTipo_,
  ...
  // Exportar función para iniciar desde app.js
